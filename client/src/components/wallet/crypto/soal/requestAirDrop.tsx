  ...
  async function requestAirDrop(e: React.FormEvent) {
    ...
    try {
      const connection = new Connection(
        import.meta.env.VITE_RPC_END_POINT,
        "confirmed",
      );
      await connection.requestAirdrop(keyPair.publicKey, balance * 1000000000);
      setBalance(0);
      setNotification(
        "Request airdrop success. Please wait for 5-10min to get balance updated.",
      );
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <div className={styles.container}>
      <div className={styles.amountSliderContainer}>
        <div className={styles.sliderValue}>{balance.toFixed(1)} SOL</div>
        <input
          type="range"
          min="0.0"
          max="5.0"
      ...
      />
      <button
        className={`${styles.button} ${styles.btnAirdrop}`}
