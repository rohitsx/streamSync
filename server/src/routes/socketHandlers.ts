...
export function handleSocketConnection(socket: Socket, io: Server) {
  ...
  socket.on(
    "createRoom",
    ({ roomId, publicKey }: { roomId: string; publicKey: string }) =>
      mySocketService.createRoom(roomId, publicKey),
  );
  ...
  socket.on(
    ...
    ({ username, publickey }: { username: string; publickey: string }) =>
      mySocketService.getSocketId({ username, publickey }),
  );
  ...
  socket.on(
    ...
    (soal: { message: string; soalQuantity: number; roomId: string }) =>
      mySocketService.primeUser(soal),
  );
  socket.on("removePrimeUser", (roomId: string) =>
    mySocketService.removePrimeUser(roomId),
  );
